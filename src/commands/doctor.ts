  ...
  const cmd = new Command('doctor')
    ...
    .action(async (options: { fix?: boolean }) => {
      ...
      for (const result of results) {
        ...
        const color = result.status === 'pass' ? picocolors.green : result.status === 'fail' ? picocolors.red : picocolors.yellow;
        logger.info(`  ${color(icon)} ${result.name.padEnd(maxNameLen)}  ${result.message}`);
      }

      logger.info('');
      logger.info(picocolors.dim(`  ${results.length} checks · ${picocolors.green(passed + ' passed')} · ${picocolors.red(failed + ' failed')} · ${picocolors.yellow(warned + ' warnings')}`));
...
async function checkNodeVersion(): Promise<DiagnosticResult> {
  ...
  return {
    ...
    message: major >= minVersion ? `v${nodeVersion}` : `v${nodeVersion} (minimum v${minVersion})`,
  };
}
...
async function checkPackageJson(): Promise<DiagnosticResult> {
  ...
  try {
    ...
    if (issues.length > 0) {
      return { name: 'package.json', status: 'warn', message: issues.join(', ') };
    }
...
async function checkTypeScript(): Promise<DiagnosticResult> {
  ...
  if (!existsSync(tsPath)) {
    return { name: 'TypeScript', status: 'warn', message: 'tsconfig.json not found' };
  }
  try {
    JSON.parse(await readFile(tsPath, 'utf-8'));
    return { name: 'TypeScript', status: 'pass', message: 'configured' };
  } catch {
    return { name: 'TypeScript', status: 'fail', message: 'invalid tsconfig.json' };
  }
}
...
async function checkDependencies(): Promise<DiagnosticResult> {
  ...
  if (found && nodeModules) {
    return { name: 'Dependencies', status: 'pass', message: `installed (${found})` };
  }
  if (!found) {
    return { name: 'Dependencies', status: 'warn', message: 'no lockfile (run install)' };
  }
  return { name: 'Dependencies', status: 'warn', message: 'not installed (run install)' };
}
