...
import { createAIProvider, type AIMessage } from '../ai/provider.js';
...
export function aiCommand(logger: Logger, _config: ForgeConfig): Command {
  const cmd = new Command('ai')
    ...
    .action(async (prompt: string | undefined, options: AIOptions) => {
      const providerType = (options.provider ?? _config.ai.provider) as 'openai' | 'anthropic' | 'none';
      ...
      const messages: AIMessage[] = [
        {
          role: 'system',
          content: 'You are Forge, an expert developer assistant integrated into a CLI tool. Provide concise, actionable code solutions. When suggesting code, explain briefly. Focus on best practices, TypeScript, and modern web development.',
        },
      ...
      if (stream) {
        ...
        const response = await provider.complete(messages, { model, maxTokens: _config.ai.maxTokens });
...
async function setupAIProvider(logger: Logger, _config: ForgeConfig): Promise<void> {
  const answers = await inquirer.prompt<{ provider: string; apiKey: string; model: string }>([
    {
      type: 'list',
      name: 'provider',
  ...
  updateConfig({
    ai: {
      ...
      model: answers.model || (answers.provider === 'openai' ? 'gpt-4o' : 'claude-3-5-sonnet-20240620'),
      maxTokens: 4096,
    },
  });
