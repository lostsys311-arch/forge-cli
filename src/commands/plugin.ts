...
export function pluginCommand(logger: Logger, pluginManager: PluginManager): Command {
  ...
  cmd
    ...
    .action(() => {
      ...
      for (const p of plugins) {
        logger.info(`  ${picocolors.cyan(p.name)}${picocolors.dim(`@${p.version}`)}  ${p.description ?? ''}`);
      }
    });
