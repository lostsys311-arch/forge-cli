...
class AnthropicProvider implements AIProvider {
  ...
  async complete(messages: AIMessage[], options?: AICompletionOptions): Promise<string> {
    ...
    this.logger.debug(`Anthropic completion: ${options?.model ?? 'claude-3-5-sonnet-20240620'}`);
    ...
    const response = await client.messages.create(completionsParams as never) as { content: { type: string; text: string }[] };
