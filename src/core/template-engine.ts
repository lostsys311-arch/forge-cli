...
import { readFiles, writeFiles, ensureDir, type FileEntry } from '../utils/file-system.js';
...
export class TemplateEngine {
  ...
  async renderTemplate(
    ...
    try {
      ...
      for (const [filePath, content] of files) {
        ...
        const template = Handlebars.compile(content, { strict: false, noEscape: true });
        const result = template(context);
        const outputPath = Handlebars.compile(filePath, { strict: false })(context);
  ...
  private registerDefaultHelpers(): void {
    ...
    this.registerHelper('capitalize', (s: string) => s.charAt(0).toUpperCase() + s.slice(1));
    this.registerHelper('camelCase', (s: string) =>
      s.replace(/[-_\s]+(.)/g, (_, c) => c.toUpperCase()).replace(/^[A-Z]/, c => c.toLowerCase())
    );
    this.registerHelper('pascalCase', (s: string) =>
      s.replace(/[-_\s]+(.)/g, (_, c) => c.toUpperCase()).replace(/^[a-z]/, c => c.toUpperCase())
    );
    ...
    this.registerHelper('includes', (arr: unknown[], val: unknown) => Array.isArray(arr) && arr.includes(val));
  }
}
